...
$proxy = new CrossProxy(
    // when this is an array, the second part will be added to the url (invisible to frontend calling the proxy) before calling the backend
    array(
        'https://theyvoteforyou.org.au',
        '/api/v1/policies/'.$policyID.'[id].json?key=s1d5YuLp2cEI%2FG%2B9NK8i'
    ),
    array(
        'localhost',
        'localhost:8080',
...
class CrossProxy {
   ...
   protected $request_cookies        = NULL;
   ...
   protected function create_request() {
      ...
      if($this->request_method === self::GET) {
         if ($this->get_srv_key('QUERY_STRING')) {
            // Brett commented out the line below - because the member ID's were being sent along to the data source, causing errors.
 ...
 */
      ...
      // Brett - These 2 lines allow for CURL on server set up's without SSL, just for testing on my local host.
   ...
   private function custom_http_parse_headers( $header ) {
      ...
      foreach( $fields as $field ) {
         if( preg_match('/([^:]+): (.+)/m', $field, $match) ) {
            // Brett - I've put the @ symbol here to supress warnings that preg_replace is being deprecated in the next PHP version
   ...
   private function trace($level, $msg) {
      ...
      if ( $level <= $this->verbose ) {
         ...
         // Brett - posix_getpid() doesn't seem to be supported on Windows
         //$content = $DateTime. " [" .  posix_getpid() ."]:[" . $level . "]" . $mylvl . " - " . $msg . "\n";
         $content = $DateTime. " [" .  getmypid() ."]:[" . $level . "]" . $mylvl . " - " . $msg . "\n";
         if (count($out_array)) {
            foreach($out_array as $key => $val) {
                ...
                $content .= $DateTime. " [" .  getmypid() ."]:[" . $level . "]" . $mylvl . " - \t\t" . $key ." => " . $val . "\n";
            }
         }
         if (is_resource($this->fp_logfile)) {
